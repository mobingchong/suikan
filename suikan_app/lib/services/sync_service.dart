...
class SyncService extends GetxService {
  ...
  /// 表现为"所有入口都刷不掉这个直播中"。
  ///
  /// 现在按房间计时：某房间只在**它自己**被真实拉取时刷新时间戳；其余房间
  /// 的时间戳照旧流逝、正常过期，从而能各自独立地退出"新鲜"状态。
  ...
  /// 快照有效期：**自动/定时轮 3 分钟**。
  ///
  /// 语义：只要"局域网内 3 分钟内有任一端拉过**该房间**"，就采用该快照，
  /// 省掉本端这一次公网请求；超过 3 分钟视为不新鲜，调用方走公网实查。
  ///
  /// 🔴 2026-09-12：
  /// - **手动刷新不再使用快照**（全量拉公网）—— 用户主动点刷新就是"要最准的"，
  ///   被快照顶替会表现为"怎么刷都刷不掉刚关播的房间"。故手动路径不传 ttl。
  /// - 新鲜度改为**按房间**判断（见 [sharedLiveStatus]）。此前整表共用时间戳，
  ///   任意房间被刷新都会给陈旧条目"续命"。
  ...
  ///
  /// [ttl] 可覆盖有效期，默认 [biliShareTtl]（3 分钟，自动/定时轮用）。
  /// **手动刷新不走这里**（改为一律拉公网）。
